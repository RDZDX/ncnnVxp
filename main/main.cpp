...
void find_cats(ncnn::Mat& in)
{
    ...
    struct ScaleInfo {
        ...
        { &cls_8,  &obj_8,  &bbox_8,   8, 12, 12 },
        { &cls_16, &obj_16, &bbox_16, 16,  6,  6 },
        { &cls_32, &obj_32, &bbox_32, 32,  3,  3 },
    };
    ...
    for (int s = 0; s < 3; s++)
    {
        ...
        int num_anchors = cls.h;  // should equal gw*gh
...
// cx, cy are offsets from grid cell — direct multiply
float cx = (grid_x + row[0]) * stride;
float cy = (grid_y + row[1]) * stride;

// w, h use exp() — standard anchor-free decoding
...
float* pB = in.channel(0);
float* pG = in.channel(1);
float* pR = in.channel(2);
...
for (int ny = 0; ny < 96; ny++)
{
    ...
    {
        if (!detected_cats[i].keep)
            continue;

        // Scale from 96x96 network space to 240x240 screen space
        float scale = 240.0f / 96.0f;

        float fx1 = detected_cats[i].x1 * scale;
        float fy1 = detected_cats[i].y1 * scale;
        float fx2 = detected_cats[i].x2 * scale;
        float fy2 = detected_cats[i].y2 * scale;
    ...
    // Frame processing — fits in 0-999, use existing lut
    ...
    // NN computing — can exceed 999, use direct formatting
